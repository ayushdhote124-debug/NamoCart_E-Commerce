...
const AdminUsers = () => {
    ...
    const thStyle = {
        padding:'15px', textAlign:'left', color:'#a1a1aa', fontSize:'0.9rem'
    }
                ...
                <tbody>
                    {users.map(u=>(
                        <tr key={u._id} style={rowStyle}>
                            ...
                            <td style={tdStyle}>
                                <span style={{ background:u.role === 'admin' ? 'rgba(234,88,12, 0.8)' : 'rgba(16,185,129,0.2)', color: u.role === 'admin' ? '#f97316': '#10b981', padding: '4px 8px', borderRadius:'4px', fontSize: '0.85rem', fontWeight:'bold'}}>{u.role.toUpperCase()}</span>
                            </td>
